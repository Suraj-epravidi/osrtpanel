...
if (isset($_POST['review_id'])) {
    ...
    if (deleteReview($conn, $review_id)) {
        ...
        header("Location: category.php?message=Review+deleted+successfully");
    } else {
        // If deletion fails, redirect with an error message
        header("Location: category.php?message=Failed+to+delete+review");
    }

    $conn->close();
} else {
    // If review_id is not set, redirect back to reviews page
    header("Location: category.php?message=No+review+ID+provided");
}
